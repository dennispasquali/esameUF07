...
export const addToCart = async (req: Request, res: Response) => {
    ...
    const findExistingOrder=await prisma.order.findFirst({
        where:{idUser: Number(data.idUser),status:"carrello",idProduct: Number(data.idProduct)},
                include:{
                    product:true
                }
        })
    ...
    if(!newCartOrder) {
        return res.status(423).send("i dati inviati sono invalidi semanticamente");
    }
...
if (existingProductRow) {
     if(data.qt+Number(existingProductRow.qt)>existingProductRow.product.qt) {
            return res.status(400).send("la qt di prodotto selezionata è troppa");
        }
     ...
     try {
        ...
        const cartOrders=await prisma.order.findMany({
             where: {
                ...
                // Includiamo la tabella pivot
                
                    include: {
                        // E dentro la pivot, includiamo i dettagli del prodotto vero
                        product: true
                    }
        ...
        cartOrders.forEach((order) => {
                  ...
                  const productId = order.idProduct; // o item.product.id
